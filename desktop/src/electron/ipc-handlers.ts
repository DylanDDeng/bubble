...
import { importBubbleHistory } from './libs/bubble-history-import';
import { readBubbleSkillContent } from './libs/provider/bubble-sdk-loader';
...
const projectTreeRequests = new Map<number, {
...
const projectWatchers = new Map<
  string,
  { watcher: FSWatcher; refresh: ReturnType<typeof createCoalescedRefresh> }
>();
...
async function createProjectEntry(
  ...
  try {
    ...
    const tree = await projectTreeReader.readFresh(projectRoot);
    if (!tree) {
...
async function handleEditLatestPrompt(
  ...
  broadcast(mainWindow, {
    ...
    payload: {
      sessionId,
      status: 'running',
      provider: session.provider || 'bubble',
...
async function emitProjectTree(mainWindow: BrowserWindow, cwd: string, scheduledVersion: number, signal: AbortSignal): Promise<void> {
  try {
    const tree = await projectTreeReader.readFresh(cwd, signal);
    if (signal.aborted || !projectWatchers.has(cwd) || getProjectTreeVersion(cwd) !== scheduledVersion) {
      return;
    }
    if (!tree) {
...
function scheduleProjectTree(cwd: string): void {
  const entry = projectWatchers.get(cwd);
  if (!entry) return;
...
export function setupIPCHandlers(mainWindow: BrowserWindow): void {
  // 初始化数据库
  sessions.initialize();
  ipcMainHandle('bubble-read-skill-content', (_event, name: string, cwd?: string) => readBubbleSkillContent(name, cwd));
  void importBubbleHistory().then(() => handleSessionList(mainWindow)).catch(error => {
    console.error('[Bubble] Could not import local history:', error instanceof Error ? error.message : error);
  });
  ...
  async function forkSessionInternal(
    ...
    try {
      ...
      const sourceProvider = (source.provider || 'bubble') as AgentProvider;
    ...
    try {
      ...
      const nameHint = branchSlugFromHint(labelHint)
        ...
        : await generateWorktreeBranchSlug({
            ...
            provider: (row.provider || 'bubble') as AgentProvider,
  ...
  const oauthOwners = new WeakSet<Electron.WebContents>();
  ipcMainHandle('start-bubble-oauth', (event, providerId: string) => {
    const result = startBubbleOAuth(providerId, event.sender.id, () => {
      // The user may have left settings while finishing browser sign-in.
      // Refresh the composer even when the settings component is unmounted.
      broadcast(mainWindow, { type: 'bubble.modelCatalogUpdated', payload: {} });
      void refreshBubbleModelCatalog().then(changed => {
        if (changed) broadcast(mainWindow, { type: 'bubble.modelCatalogUpdated', payload: {} });
      });
    });
    const ownerId = event.sender.id;
    if (!oauthOwners.has(event.sender)) {
      oauthOwners.add(event.sender);
      event.sender.once('destroyed', () => cancelBubbleOAuth(ownerId));
    }
    return result;
  });
  ipcMainHandle('get-bubble-oauth-state', () => getBubbleOAuthState());
  ipcMainHandle('cancel-bubble-oauth', () => cancelBubbleOAuth());
  ipcMainHandle('reopen-bubble-oauth', () => reopenBubbleOAuth());
  ipcMainHandle('logout-bubble-oauth', (_event, providerId: string) => logoutBubbleOAuth(providerId));
  ...
  ipcMainHandle('get-project-tree', async (event, cwd: string, requestId?: string) => {
    ...
    let requests = projectTreeRequests.get(sender.id);
    if (!requests) {
      const controllers = new Map<string, AbortController>();
      const onDestroyed = () => {
        for (const controller of controllers.values()) controller.abort();
        projectTreeRequests.delete(sender.id);
      };
      requests = { controllers, onDestroyed };
      projectTreeRequests.set(sender.id, requests);
      sender.once('destroyed', onDestroyed);
    }
    const id = requestId || uuidv4();
    requests.controllers.get(id)?.abort();
    const controller = new AbortController();
    requests.controllers.set(id, controller);
    try {
      return await readProjectTree(cwd, controller.signal);
    } catch (error) {
      if (controller.signal.aborted) return null;
      throw error;
    } finally {
      if (requests.controllers.get(id) === controller) requests.controllers.delete(id);
      if (!requests.controllers.size) {
        sender.removeListener('destroyed', requests.onDestroyed);
        if (projectTreeRequests.get(sender.id) === requests) projectTreeRequests.delete(sender.id);
      }
    }
  });

  ipcMainHandle('cancel-project-tree-read', (event, requestId: string) => {
    projectTreeRequests.get(event.sender.id)?.controllers.get(requestId)?.abort();
    return true;
  });
  ...
  ipcMainHandle('watch-project-tree', async (_event, cwd: string) => {
    ...
    try {
      ...
      const refresh = createCoalescedRefresh(
        signal => emitProjectTree(mainWindow, cwd, getProjectTreeVersion(cwd), signal),
        PROJECT_TREE_REFRESH_DELAY_MS,
        error => console.error('Failed to refresh project tree:', error),
      );
      projectWatchers.set(cwd, { watcher, refresh });
      scheduleProjectTree(cwd);
      return true;
    } catch (error) {
...
function buildSessionInfoFromRow(
  ...
  return {
    ...
    source:
      row.session_origin === 'bubble_imported' ? 'bubble_imported' : row.session_origin === 'claude_remote'
        ? 'claude_remote'
        : row.provider === 'codex'
          ? 'codex_local'
...
async function handleSessionStart(
  ...
  const referenceContext = appendSessionReferences('', prompt, undefined, provider || 'bubble');
  const chosenProvider = provider || 'bubble';
...
async function handleSessionContinue(
  ...
  const referenceContext = appendSessionReferences('', prompt, sessionId, provider || session.provider || 'bubble');
  ...
  const existingEntry = runnerHandles.get(sessionId);
  const previousProvider = session.provider || 'bubble';
...
function startRunner(
  ...
  const provider = providerOverride || session.provider || 'bubble';
...
async function handleRunnerPrewarm(
  ...
  if (!initial) return;
  const prewarmProvider = initial.provider || 'bubble';
...
function handleMcpGetConfig(mainWindow: BrowserWindow, projectPath?: string): void {
  ...
  const codexGlobalServers = {};
  const opencodeGlobalServers = {};
  const opencodeProjectServers = {};
  const kimiGlobalServers = {};
  const kimiProjectServers = {};
  const qoderGlobalServers = {};
  const bubbleGlobalServers = getBubbleMcpServers();
  const deepseekGlobalServers = {};
  const deepseekProjectServers = {};
...
export function cleanup(): void {
  cancelBubbleOAuth();
  for (const cwd of projectWatchers.keys()) closeProjectTreeWatcher(cwd);
  for (const requests of projectTreeRequests.values()) {
    for (const controller of requests.controllers.values()) controller.abort();
  }
  projectTreeReader.dispose();
  ipcMain.removeAllListeners('client-event');
  automationScheduler?.stop();
