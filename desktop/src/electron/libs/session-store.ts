...
function normalizeAutomationProvider(value?: string | null): AgentProvider {
  if (value && value !== 'bubble') throw new Error('Automations use the Bubble agent.');
  return 'bubble';
}
...
export function createSession(params: {
  ...
  if ((params.provider || 'bubble') === 'claude') {
    invalidateClaudeUsageReportCache();
  }
...
// 获取所有会话
export function importBubbleHistorySnapshot(
  source: { name: string; cwd?: string; title: string; preview: string; mtime: number },
  readMessages: () => StreamMessage[],
): boolean {
  return getDb().transaction(() => {
    if (getDb().prepare('SELECT id FROM sessions WHERE bubble_session_id = ?').get(source.name)) return false;
    const messages = readMessages();
    if (!messages.length) return false;
    const row = createSession({ provider: 'bubble', title: source.title || source.preview || 'New Chat', cwd: source.cwd });
    for (const message of messages) {
      addMessage(row.id, { ...message, createdAt: message.createdAt ?? source.mtime });
    }
    getDb().prepare(`UPDATE sessions SET bubble_session_id = ?, session_origin = 'bubble_imported', status = 'idle', todo_state = 'review',
      created_at = ?, updated_at = ? WHERE id = ?`).run(source.name, messages[0]?.createdAt ?? source.mtime, source.mtime, row.id);
    return true;
  })();
}
