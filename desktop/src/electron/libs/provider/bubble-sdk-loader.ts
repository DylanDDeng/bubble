...
export { resolveBubbleHome } from '../bubble-home';
/**
 * Bubble SDK loader.
 *
...
export type BubbleProviderRegistry = {
  getOAuthLoginKeys(id: string): string[];
  getAuthStorage(): {
    set(id: string, credentials: { type: "oauth" } & import("../bubble-oauth-protocol").BubbleOAuthTokens): void;
...
export function loadBubbleSdk(): Promise<BubbleSdkModule> {
  if (!sdkPromise) {
    sdkPromise = importEsm(pathToFileURL(join(app.getAppPath(), 'runtime/bubble/dist/sdk/index.js')).href);
    sdkPromise.catch(() => {
      sdkPromise = null;
    });
  }
...
export function loadBubbleProviderCatalog(): Promise<BubbleProviderCatalogModule> {
  if (!catalogPromise) {
    catalogPromise = (importEsm as unknown as (s: string) => Promise<BubbleProviderCatalogModule>)(
      pathToFileURL(join(app.getAppPath(), 'runtime/bubble/dist/provider-registry.js')).href
    );
