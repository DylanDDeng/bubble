...
import { resolveBubbleHome, getBubbleSdk } from './provider/bubble-sdk-loader';
...
async function probeBubble(): Promise<AgentRuntimeEntry> {
  try {
    ...
    return sdk.getModelConfig().providers.some(provider => provider.hasApiKey)
      ? entry('bubble', 'ready', { summary: 'Bubble is ready.' })
      : entry('bubble', 'login_required', { summary: 'Add a model provider to get started.', detail: 'Open Settings → Providers to configure Bubble.' });
  } catch (error) {
    return entry('bubble', 'error', { summary: 'Could not load Bubble.', detail: error instanceof Error ? error.message : String(error) });
  }
}
...
export async function getAgentRuntimeDirectory(force = false): Promise<AgentRuntimeDirectoryReport> {
  const probes: Array<[AgentProvider, Promise<AgentRuntimeEntry>]> = [['bubble', probeBubble()]];
