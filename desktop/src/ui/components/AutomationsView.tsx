import { useBubbleModelConfig } from '../hooks/useBubbleModelConfig';
...
const PROVIDERS: Array<{ id: AgentProvider; label: string }> = [{ id: 'bubble', label: 'Bubble' }];
...
export function AutomationsView() {
  ...
  const bubbleConfig = useBubbleModelConfig();
  ...
  const modelOptions = useMemo<Partial<Record<AgentProvider, string[]>>>(() => {
    ...
    return {
      bubble: unique([bubbleConfig.defaultModel, ...bubbleConfig.options]),
    };
  }, [bubbleConfig]);
  ...
  const openNewDialog = (template?: (typeof TEMPLATES)[number]) => {
    const defaultModel = modelOptions.bubble?.[0] || '';
    setForm({
          ...
          <header className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-[13px] leading-5 text-[var(--text-secondary)]">
                Schedule Bubble to run prompts in a selected project. Each run creates a normal project session.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
            ...
            <Field label="Prompt">
              <textarea
                ...
                placeholder="Describe the recurring work Bubble should perform..."
              />
            </Field>
