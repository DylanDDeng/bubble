          ...
          <PropertyGroup label="Properties">
            ...
            <PropertyRow label="Created">
              <span className="text-[12px] text-[var(--text-secondary)]">
                {relativeTimeAgo(task.createdAt)}
              </span>
            </PropertyRow>
            <PropertyRow label="Updated">
              <span className="text-[12px] text-[var(--text-secondary)]">
                {relativeTimeAgo(task.updatedAt)}
              </span>
            </PropertyRow>
          </PropertyGroup>
...
function EventRow({ event, profileName }: { event: BoardTaskEvent; profileName: string | null }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[12px] text-[var(--text-muted)]">
      ...
      <span className="flex-shrink-0 text-[11px]">{relativeTimeAgo(event.at)}</span>
    </div>
  );
}
