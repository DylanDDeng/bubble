...
const SETTINGS_NAV_GROUPS: { label: string; tabs: SettingsTabKey[] }[] = [
  ...
  { label: 'Integrations', tabs: ['browser', 'mcp', 'providers'] },
];
...
export function Settings() {
  ...
  const mcpCatalogs = [store.mcpBubbleGlobalServers];
  ...
  const searchResults = findSettings(search, extraEntries).filter(entry => entry.tab !== 'bridge' && (!entry.runtime || entry.runtime === 'bubble') && !/claude|codex|deepseek harness/i.test(`${entry.label} ${entry.scope || ''}`));
  ...
  const resolvedActiveSettingsTab: SettingsTabKey = isSettingsTabKey(activeSettingsTab) && activeSettingsTab !== 'bridge'
    ? activeSettingsTab
    : 'general';
    ...
    <div className="aegis-settings flex h-full min-h-0 min-w-0 flex-col bg-[var(--bg-primary)]">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto bg-[var(--bg-primary)] select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
        ...
        >
          ...
          {resolvedActiveSettingsTab === 'usage' && <BubbleUsageSettingsContent />}
