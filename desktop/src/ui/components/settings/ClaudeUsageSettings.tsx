...
export function BubbleUsageSettingsContent() {
  const [state, setState] = useState<ProviderUsageState>({report: null, loading: true, error: null});
  useEffect(() => {
    let cancelled = false;
    window.electron.getAgentUsageReport('bubble', 365).then(report => {
      if (!cancelled) setState({report, loading: false, error: null});
    }).catch(error => {
      if (!cancelled) setState({report: null, loading: false, error: normalizeUsageLoadError(error, 'get-agent-usage-report')});
    });
    return () => { cancelled = true; };
  }, []);
  const card: UsageProviderCard = {id: 'bubble', title: 'Bubble', logo: null, ...state};
  const report = state.report;
  const stats = useMemo(() => report ? computeUsageStats(report) : null, [report]);
  return <div className="space-y-8 pb-10">
    <p className="text-[13px] text-[var(--text-secondary)]">Usage recorded by Bubble Desktop over the last year. Imported conversations without usage records are excluded.</p>
    {renderUsageState(card) || (report && stats && <>
      <UsageStatStrip stats={stats} costMode={report.costMode} note={report.note} />
      <TokenActivitySection daily={report.daily} />
      <div className="grid grid-cols-1 gap-x-12 gap-y-8 md:grid-cols-2">
        <ActivityInsights report={report} stats={stats} />
        <TopModels report={report} />
      </div>
    </>)}
  </div>;
}
          ...
          top.map((model) => (
            <div key={model.model} className="flex items-center justify-between gap-4 py-2.5">
              <div className="flex min-w-0 items-center gap-2.5">
                <ModelProviderLogo model={model.model} />
...
function ModelProviderLogo({ model }: { model: string }) {
  // Bubble records provider-qualified IDs. Use the same provider artwork and
  // fallback as Settings > Providers, including subscription/plan aliases.
  const providerId = model.trim().match(/^([^:/]+)[:/]/)?.[1].toLowerCase();
  if (providerId) {
    return <ProviderLogo providerId={providerId} name={providerId} />;
  }

  // Older reports may contain only a model name or a display label.
  const logo = getProviderLogoForModel(model);
  return logo
    ? <img src={logo} alt="" className="h-4 w-4 flex-shrink-0" aria-hidden="true" />
    : <ProviderLogo providerId={model} name={model || '?'} />;
}
