...
window.electron={
 ...
 getProjectGitSummary:async()=>({isGitRepository:true}),getProjectTree:async()=>null,cancelProjectTreeRead:async()=>true,
...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  const logo=await rect('.bubble-new-thread-logo');
  assert.equal(logo.width,64);assert.equal(logo.height,64);
  ...
  assert.equal(await js('getComputedStyle(document.querySelector(".bubble-new-thread-logo")).opacity'),'0.72');
  await js('document.querySelector(".bubble-new-thread-logo").dispatchEvent(new PointerEvent("pointerup",{bubbles:true,button:0,pointerType:"mouse"}))');
  await delay(150);
  assert.notEqual(await js('getComputedStyle(document.querySelector(".bubble-new-thread-logo")).transform'),'none','logo responds to pointer');
  await delay(1200);
  ...
  assert.equal(await js('getComputedStyle(document.querySelector(".bubble-new-thread-logo img")).filter'),'grayscale(1)','cat keeps the muted welcome style in dark mode');
  ...
  await checkContextTray();
  assert.equal(await js('document.querySelectorAll(".bubble-new-thread-logo").length'),1,'draft entry uses the same home logo');
