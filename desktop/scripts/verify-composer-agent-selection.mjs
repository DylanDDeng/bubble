...
assert.ok(
  promptInput.includes('onSelectionChange: handleSessionAgentSelectionChange') &&
    ...
    promptInput.includes("agentSelection.selectModel(option, 'bubble')") &&
    promptInput.includes("selectAgentConfiguration({ ...change, provider: 'bubble' })") &&
    !promptInput.includes('handoffTarget') &&
    !promptInput.includes('handoffSessionToProvider'),
  'Bubble picker changes stay in the same agent, persist atomically, and never offer agent handoff'
);
