...
assert.ok(
  packageJson.scripts.build.includes('sync-bubble-sdk.mjs'),
  'Desktop must build against the local Bubble SDK'
);
...
assert.ok(
  read('src/electron/ipc-handlers.ts').includes("ipcMainHandle('bubble-list-skills'") &&
    ...
    !skillLibrary.includes("ClaudeLibraryContent") &&
...
assert.ok(
  providerUtils.includes("{ id: 'bubble', label: 'Bubble' }") &&
    providerUtils.includes("return 'bubble'") &&
