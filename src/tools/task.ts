...
import type { SubtaskType } from "../agent/subtask-policy.js";

export function createTaskTool(): ToolRegistryEntry {
  return {
    name: "task",
    readOnly: true,
...
Do not use this for edits or shell-heavy workflows. The subtask agent runs in read-only plan mode.`,
    ...
    async execute(args, ctx): Promise<ToolResult> {
      ...
      const subtaskType = typeof args.subtaskType === "string" ? args.subtaskType as SubtaskType : undefined;
      if (!prompt) {
