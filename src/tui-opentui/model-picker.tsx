...
function adaptKeyEvent(key: {
  ...
  if (key.name === "up") flags.upArrow = true;
  if (key.name === "down") flags.downArrow = true;
  if (key.name === "return") flags.return = true;
  if (key.name === "escape") flags.escape = true;
  if (key.name === "backspace") flags.backspace = true;
  if (key.name === "delete") flags.delete = true;
...
export function ModelPicker({ registry, current, recent, onSelect, onCancel }: ModelPickerProps) {
  ...
  return (
    ...
    >
      <text fg={theme.accent} attributes={1}>Select Model</text>
      <SearchField query={query} placeholder="Type to search models..." />
      <text fg={theme.muted}>↑/↓ navigate · Enter select · Esc cancel · Backspace clear</text>
      <box style={{ flexDirection: "column", marginTop: 1 }}>
        {options.length === 0 && (
          <text fg={theme.muted}>No models match "{query}"</text>
        )}
        {visible.map((opt, i) => {
          const actualIndex = start + i;
          const isSelected = actualIndex === selectedIndex;
          return (
            <box key={opt.id} style={{ flexDirection: "row" }}>
...
function SearchField({ query, placeholder }: { query: string; placeholder: string }) {
  ...
  return (
    <box style={{ marginTop: 1, marginBottom: 1, flexDirection: "row" }}>
      <text fg={theme.accent}>{"❯ "}</text>
      <text>{query}</text>
      <text fg={cursorVisible ? "black" : theme.accent} bg={cursorVisible ? theme.accent : undefined}> </text>
      {!query && <text fg={theme.muted}> {placeholder}</text>}
    </box>
  );
}
...
export function ProviderPicker({ providers, current, onSelect, onCancel, title }: ProviderPickerProps) {
  ...
  return (
    ...
    >
      <text fg={theme.accent} attributes={1}>{title || "Select Provider"}</text>
      <text fg={theme.muted}>↑/↓ navigate · Enter select · Esc cancel · type letter to jump</text>
      <box style={{ flexDirection: "column", marginTop: 1 }}>
        {visible.map((p, i) => {
          const actualIndex = start + i;
          const isSelected = actualIndex === selectedIndex;
...
export function KeyPicker({ providerName, onSubmit, onCancel }: KeyPickerProps) {
  ...
  return (
    ...
    >
      <text fg={theme.accent} attributes={1}>Enter API Key for {providerName}</text>
      <text fg={theme.muted}>Paste or type the key · Enter to submit · Esc to cancel</text>
...
export function SkillPicker({ skills, onSelect, onCancel }: SkillPickerProps) {
  ...
  return (
    ...
    >
      <text fg={theme.accent} attributes={1}>Select Skill</text>
      <SearchField query={query} placeholder="Type to search skills..." />
      <text fg={theme.muted}>↑/↓ navigate · Enter load · Esc cancel · Backspace clear</text>
      <box style={{ flexDirection: "column", marginTop: 1 }}>
        {options.length === 0 && (
          <text fg={theme.muted}>No skills match "{query}"</text>
        )}
        {visible.map((skill, i) => {
          const actualIndex = start + i;
          const isSelected = actualIndex === selectedIndex;
