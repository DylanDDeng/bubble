...
function hasVisibleDisplayMessage(message: DisplayMessage): boolean {
  if (message.syntheticKind === "ui_summary") return false;
  if (message.syntheticKind === "ui_compact_summary") return true;
  if (message.role === "user" || message.role === "error") return message.content.trim().length > 0;
  return !!(
...
export function App({ agent, args, sessionManager, createProvider, registry, skillRegistry, planHandlerRef, approvalHandlerRef, questionController, bashAllowlist, settingsManager, lspService, mcpManager, themeMode: initialThemeMode, themeOverrides, detectedTheme, onThemeModeChange, flushMemory, runMemoryCompaction, runMemorySummary, runMemoryRefresh, bypassEnabled, onExit }: AppProps) {
  ...
  const exit = useCallback(() => {
    try {
      (renderer as RendererLike & { destroy?: () => void }).destroy?.();
    } catch {
  ...
  return (
    <ThemeProvider value={palette}>
      <box style={{ flexDirection: "column", height: terminalRows, backgroundColor: palette.background }}>
        {showHomeSurface ? (
          <HomeSurface
            terminalColumns={terminalColumns}
        ...
        <box style={{ flexDirection: "column", paddingLeft: 1, paddingRight: 1, paddingTop: 1, flexShrink: 0 }}>
          ...
          <MessageList
            ...
            nowTick={nowTick}
            welcomeBanner={showHomeSurface ? null : welcomeBannerNode}
          />
        {pickerMode === "model" && (
          <ModelPicker
            registry={safeRegistry}
      ...
      {!isExiting && isRunning && !pickerMode && !pendingPlan && !pendingApproval && !pendingQuestion && !pendingFeedback && (
        <box style={{ paddingLeft: 1, paddingRight: 1, paddingBottom: 1, flexShrink: 0 }}>
          <WaitingIndicator
      ...
      {!isExiting && !pickerMode && (
        <box style={{ paddingBottom: 1, flexShrink: 0 }}>
          {composerNode}
        </box>
      )}
      {!isExiting && (
...
function SessionHeader({
  ...
  return (
    <box style={{ flexDirection: "row", marginBottom: 1 }}>
      <text fg={theme.textMuted} content={left} />
      <text fg={theme.textDim} content={filler} />
      <text fg={mode === "bypassPermissions" ? theme.warning : theme.accent} attributes={1} content={right} />
    </box>
  );
}
...
function WaitingIndicator({
  ...
  return (
    <box>
      <text fg={theme.accent}>{SPINNER_FRAMES[frameIndex]}</text>
      <text fg={theme.muted}> {phrase} </text>
      <text fg={theme.muted}>
